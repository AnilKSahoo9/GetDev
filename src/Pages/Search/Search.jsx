...
const Search = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (query2) {
      retrieveDataFromServer(1, query2);
    } else {
  ...
  const retrieveDataFromServer = (updatedPageNo = "", query) => {
    ...
    axios
      .get(
        `http://localhost:4000/api/users/search/${query1}/${query2}${pageNumber}`
      )
      .then((responseData) => {
        console.log(responseData);
        const resultNotFoundMsg = !responseData.data.user.length
          ? "there re no more search results.plz try a new search"
          : "";
  ...
  const handlePageClick = (type, Event) => {
    ...
    const updatePageNo =
      "prev" === type ? currentPageNo - 1 : currentPageNo + 1;
    if (!loading) {
      setLoading(true);
      retrieveDataFromServer(updatePageNo, query2);
    }
  };
          ...
          <Form.Group>
            <Form.Select
              ...
              options={[
                ...
                { text: "Work Profile", value: "work_profile" },
              ]}
              placeholder="Filter"
